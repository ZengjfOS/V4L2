...
{
    unsigned int in, out;
    int y0, u, y1, v;   // r g b x
    // unsigned int pixel24;
    // unsigned char *pixel = (unsigned char *)&pixel24;
    unsigned int size = width*height*4;

    for (in = 0, out = 0; in < size; in += 4, out += 3)
    {
      /*特殊说明，因bmp文件是以bgr顺序排列而不是rgb，故此u，v颠倒*/
        y0 = yuv[in+0];                         // b
        v  = yuv[in+1];//u  = yuv[in+1];        // g
        y1 = yuv[in+2];                         // r
        u  = yuv[in+3];//v  = yuv[in+3];        // x
...
int main()
{
  ...
  unsigned char yuyv[WIDTH*HEIGHT*4],rgb[WIDTH*HEIGHT*3];
  /*打开yuyv文件并读取*/
  ...
  fread(yuyv,1,WIDTH*HEIGHT*4,fd_yuv);
  fclose(fd_yuv);
  /*yuyv转rgb格式*/
